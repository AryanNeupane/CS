...
**Date:** 27/11/2082
**API Base URL:** [https://lec-backend-gva9.onrender.com](https://lec-backend-gva9.onrender.com)
...
| **TC_10**    | Verify trimming spaces in username                | Login successful; token + user info.                                 | Login successful; token generated.                   | ✅ Pass          | Trimming works.               |
| **TC_11**    | Verify username case sensitivity                  | Login should work regardless of case.                                | Login successful; token generated.                   | ✅ Pass          | Case-insensitive login.       |
...
| **TC_13**    | Verify incorrect endpoint                         | 404 Not Found                                                        | Returns 401 Unauthorized                             | ❌ Fail          | Wrong status code handling.   |
...
| **TC_15**    | Verify account lock mechanism                     | 423 error — account locked.                                          | 423 error — account locked.                          | ✅ Pass          | Lockout works.                |
| **TC_16**    | Verify no sensitive info leaked in errors         | 401 error — generic message.                                         | 401 error — invalid credentials.                     | ✅ Pass          | No sensitive info exposed.    |
| **TC_17**    | Verify different Content-Types (non-JSON)         | 415 error — unsupported media type.                                  | 400 error — unsupported Content-Type.                | ⚠️ Partial Pass | Status mismatch.              |
| **TC_18**    | Verify login with special characters              | 401 error — invalid credentials.                                     | 401 Unauthorized — invalid credentials.              | ✅ Pass          | Correct rejection.            |
| **TC_19**    | Verify login response headers                     | `content-type: application/json` + `cache-control: no-store` present | Headers returned correctly                           | ✅ Pass          | Correct headers.              |
| **TC_20**    | Verify JWT validity (role + exp claims)           | Should return valid token payload                                    | Returns correct data                                 | ✅ Pass          | Token valid.                  |
...
* Rate-limit and lockout mechanisms working correctly
